...
from analysis import accuracy, binary_accuracy
...
from classes import Meters, Experiment, RunRecord, N_CLASSES, N_ATTRIBUTES
import configs as cfgs

from utils import upload_to_aws, get_secrets, download_from_aws
from tti import run_tti
    ...
    for data in loader:
        ...
        with torch.no_grad():
            if attr_preds is not None:
                ...
                label_ndxs = model.make_batch_ndxs(attr_labels.shape[0], batch_attr_mask)
                ...
                attr_pred_sigmoids = attr_pred_sigmoids.reshape(-1, args.n_attributes)
...
def train(
    ...
    run_save_path = Path(args.run_save_path)
    model.train()
    model = model.cuda()
...
def train_multi(args: Experiment) -> None:
    ...
    if args.load is not None:
        model = load_model(args, Multimodel)
    else:
...
def train_switch(args):
    ...
    now_str = datetime.now().strftime(DATETIME_FMT)
    args.run_save_path = os.path.join(args.log_dir, args.tag, now_str)

    if not os.path.exists(args.run_save_path):
        os.makedirs(args.run_save_path, exist_ok=True)  
...
# ok so we're going to have an optional argument that gets an entire list of configs that can be run either sequentially or in parallel



if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--cfg-index', type=int, default=0, help='Index of config to run, set to -1 to run all configs in list sequentially (may be very slow).')
    parser.add_argument('--cfg-list', type=str, default=None, help='Name of config list to run, if any.')
    args = parser.parse_args()
     
    if args.cfg_list is not None:
        if args.cfg_list in cfgs.cfg_lists.keys():
            configs = cfgs.cfg_lists[args.cfg_list]
        else:
            print(f"Config list {args.cfg_list} not found, running default config list.")
